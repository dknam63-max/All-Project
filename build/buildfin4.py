...
# bottom scrim for legibility on any photo
sd=ImageDraw.Draw(card)
for y in range(1050,H):
    a=int(165*min(1,(y-1050)/360)); sd.line([(0,y),(W,y)],fill=(12,16,14,a))
d=ImageDraw.Draw(card)
fb=ImageFont.truetype("NG.ttf",104); fm=ImageFont.truetype("NG.ttf",52); fp=ImageFont.truetype("NG.ttf",50)
x0,y0,x1,y1=70,1320,1010,1720
# soft shadow then cream card
d.rounded_rectangle([x0+8,y0+12,x1+8,y1+12],radius=52,fill=(0,0,0,90))
d.rounded_rectangle([x0,y0,x1,y1],radius=52,fill=(250,247,238,252))
def ctext(y,txt,fnt,col):
    w=d.textlength(txt,font=fnt); d.text(((W-w)/2,y),txt,font=fnt,fill=col)
ctext(y0+46,"에어클린매트",fb,(28,28,28,255))
ctext(y0+180,"현관 인테리어 추천",fm,(95,95,95,255))
pt="네이버 스마트스토어 검색"; pw=d.textlength(pt,font=fp); pad=40
px0=(W-pw)/2-pad; px1=(W+pw)/2+pad; py0=y0+258; py1=py0+90
d.rounded_rectangle([px0,py0,px1,py1],radius=45,fill=(3,199,90,255))
# little search glyph
d.ellipse([px0+34,py0+30,px0+58,py0+54],outline=(255,255,255,255),width=6)
d.line([px0+55,py0+51,px0+66,py0+62],fill=(255,255,255,255),width=6)
d.text(((W-pw)/2+22,py0+20),pt,font=fp,fill=(255,255,255,255))
card.save("card.png")
...
run(["ffmpeg","-y","-i","vsub.mp4","-i","mix.wav","-map","0:v","-map","1:a","-c:v","copy","-c:a","aac","-b:a","192k","-shortest","/home/user/ACM_02_final3_916.mp4"])
print("DONE", os.path.getsize("/home/user/ACM_02_final3_916.mp4"))
