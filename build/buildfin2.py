...
imgseg("6_final.png",3.0,72,"min(1.0+0.0005*on,1.05)","s6.mp4")
...
run(["ffmpeg","-y","-i","vid.mp4","-i","card.png","-filter_complex",
     "[1:v]format=rgba,fade=t=in:st=17.2:d=0.4:alpha=1[c];[0:v][c]overlay=0:0:enable='gte(t,17.2)'[v]","-map","[v]",*E,"vidc.mp4"])
...
# BGM
import numpy as np
sr=44100; dur=20.0; N=int(sr*dur); t=np.arange(N)/sr
prog=[[261.63,329.63,392.00],[196.00,246.94,392.00],[220.00,261.63,329.63],[174.61,220.00,349.23]]
bgm=np.zeros(N)
for i,ch in enumerate(prog):
    s=int(i*5*sr); e=int(min((i+1)*5,dur)*sr); tt=np.arange(e-s)/sr; seg=np.zeros(e-s)
...
for k,ms in offs.items():
    run(["ffmpeg","-y","-i",k+".mp3","-af",f"aresample=44100,adelay={ms}|{ms},apad","-t","20","-ac","2","-ar","44100",k+"_d.wav"]); parts.append(k+"_d.wav")
cmd=["ffmpeg","-y"]
for p in parts: cmd+=["-i",p]
cmd+=["-filter_complex",f"amix=inputs={len(parts)}:duration=longest:normalize=0,volume=2.0,aresample=44100[nar]","-map","[nar]","-t","20","-ac","2","-ar","44100","nar.wav"]
run(cmd)
...
run(["ffmpeg","-y","-i","vsub.mp4","-i","mix.wav","-map","0:v","-map","1:a","-c:v","copy","-c:a","aac","-b:a","192k","-shortest","/home/user/ACM_02_final_916.mp4"])
print("DONE", os.path.getsize("/home/user/ACM_02_final_916.mp4"))
