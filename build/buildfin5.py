...
fb=ImageFont.truetype("NG.ttf",80); fm=ImageFont.truetype("NG.ttf",42); fp=ImageFont.truetype("NG.ttf",44)
# compact card placed on the wood-floor gap between the mat (top edge ~y820) and the living room
x0,y0,x1,y1=140,556,940,812
d.rounded_rectangle([x0+7,y0+11,x1+7,y1+11],radius=46,fill=(0,0,0,85))
d.rounded_rectangle([x0,y0,x1,y1],radius=46,fill=(250,247,238,252))
def ctext(y,txt,fnt,col):
    w=d.textlength(txt,font=fnt); d.text(((W-w)/2,y),txt,font=fnt,fill=col)
ctext(y0+28,"에어클린매트",fb,(28,28,28,255))
ctext(y0+126,"현관 인테리어 추천",fm,(95,95,95,255))
pt="네이버 스마트스토어 검색"; pw=d.textlength(pt,font=fp); pad=50
px0=(W-pw)/2-pad; px1=(W+pw)/2+pad; py0=y0+176; py1=py0+80
d.rounded_rectangle([px0,py0,px1,py1],radius=40,fill=(3,199,90,255))
d.ellipse([px0+34,py0+26,px0+56,py0+48],outline=(255,255,255,255),width=6)
d.line([px0+53,py0+45,px0+64,py0+56],fill=(255,255,255,255),width=6)
d.text(((W-pw)/2+30,py0+16),pt,font=fp,fill=(255,255,255,255))
card.save("card.png")
...
run(["ffmpeg","-y","-i","vsub.mp4","-i","mix.wav","-map","0:v","-map","1:a","-c:v","copy","-c:a","aac","-b:a","192k","-shortest","/home/user/ACM_02_final4_916.mp4"])
print("DONE", os.path.getsize("/home/user/ACM_02_final4_916.mp4"))
